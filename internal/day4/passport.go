...
func (p Passport) IsStrictValid() bool {
  ...
  if err != nil {
    log.Fatalf("converting birth year to int: %w", err)
  }
  ...
  if err != nil {
    log.Fatalf("converting issue year to int: %w", err)
  }
  ...
  if err != nil {
    log.Fatalf("converting expiration year to int: %w", err)
  }
  ...
  if strings.HasSuffix(p.Height, "cm") {
    ...
    if err != nil {
      log.Fatalf("converting cm height to int: %w", err)
    }
  ...
  if strings.HasSuffix(p.Height, "in") {
    height, err := strconv.ParseInt(p.Height[:len(p.Height)-2], 10, 64)
    if err != nil {
      log.Fatalf("converting in height to int: %w", err)
    }
